...
object Companion {
    const val siteUrl = "https://gamecraft.fun/gameui50/"
    const val offerUrl = "https://webnexa.site/webnexa_apps/T4/_OfferWall/"
    const val timewallUrl = "https://timewall.io/users/login?oid=048e1332b6b597eb&tab=tasks&uid="
    const val siteAdsUrl = "https://earningads.io/_CustomsAds/"
    const val ADS_UNIQUE_APP_ID = "BLOCKMAGICPUZZLE523"
    const val APP_VERSION = 1
    const val APP_ID = 13
    const val PUBSCALE_APP_ID = "80120313"
    const val ADJUMP_APP_ID = "322836"
    const val ADJUMP_AC_ID = "57264"
}
...
object Utils {
    ...
    fun playPopSound(context: Context) {
        if (TinyDB.getBoolean(context, "isSoundOn", true)) {
            val mediaPlayer = MediaPlayer.create(context, R.raw.popup_sound)
            mediaPlayer.start()
    ...
    fun winPopSound(context: Context) {
        if (TinyDB.getBoolean(context, "isWinSound", true)) {
            val mediaPlayer = MediaPlayer.create(context, R.raw.cashier)
            mediaPlayer.start()
            mediaPlayer.setOnCompletionListener {
                it.release()
            }
